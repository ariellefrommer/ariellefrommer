...
class TimeEvolution:
    ...
    def plot_number_operators(self, file_path):
        ...
        # filtering out the indices with non-zero variance
        valid_indices = [idx for idx in bins if np.var(self.obs_store_full[:, idx]) > 1e-5]
        
        # generating colors from the colormap
        colors = [colormap(i / len(valid_indices)) for i in range(len(valid_indices))]
        
        # plotting number operators
        for idx, color in zip(valid_indices, colors):
            label = labels[idx]
    ...
    def plot_eq(self, file_path):
        ...
        for idx, valid_idx in enumerate(valid_indices):
            ...
            color = colors[idx]
            ax = axs[idx]
            ax.plot(self.time, self.obs_store_full[:, valid_idx], label=label, color=color)
            ax.axhline(y=self.eq_values[valid_idx], label=f'{label} eq.', color=color, linestyle='--', linewidth=2)
    ...
    def plot_eq_diff(self, file_path):
        ...
        # filtering out the indices with non-zero variance
        valid_indices = [idx for idx in bins if np.var(self.eq_diff[:, idx]) > 1e-5]
